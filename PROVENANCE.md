...
| v2.27.1 | 2026-07-27 | Codex | GPT-5 / Codex Desktop | @jonathanwxh-cell | Fixed the live gamer-review findings with a protected 360×640 combat crop/tutorial lane, one combined Oath-chain/IRONBOUND plate, proactive Sunder copy, stronger partial/full Resolve presentation, desktop Break guidance, and deterministic short-phone/combined-status acceptance; preserved every combat, difficulty, score, save, audio, asset, and touch-target contract |
| v2.27.2 | 2026-07-28 | Codex | GPT-5 / Codex Desktop | @jonathanwxh-cell | Fixed the focused combo review findings: pending Sunder can no longer steal stagger Execute, one deliberate input survives wound recovery with roll priority, contact-authoritative HUD/button states name the next action and explain route loss, pause retains technique recipes, deterministic gameplay plus phone-capture acceptance preserves the 600 ms window and all balance/save/audio/asset contracts, repeated acceptance evidence hardened suspended-context charge cleanup plus permanent-deck reuse at the existing crossfade settle point, score-dialog focus now returns in the committed layout, and the shared-voice lifecycle probe is isolated from real boss cues |
| v2.27.3 | 2026-07-28 | Codex | GPT-5 / Codex Desktop | @jonathanwxh-cell | Fixed the reported inert touch BREAK command by latching one full-Resolve tap through the existing 0.5-second charged release; preserved desktop partial holds, Sunder/Execute priority, all balance/save/audio/render contracts, and added a real-touch ready/charging/released regression with phone screenshots |
| v2.27.4 | 2026-07-29 | Codex + five gamer-role agents | GPT-5 / Codex Desktop | @jonathanwxh-cell | Researched and repaired first-run orientation without adding systems: preserved the full playable tutorial clock, gave only unfinished Journey runs one readable opening approach, retained the touch MOVE guide until movement, shortened the roll lesson, corrected device-specific technique copy, and accepted the amended candidate through five independent 8.0–9.0/10 role scorecards plus the complete zero-error QA matrix |
| v2.27.5 | 2026-07-29 | Codex + combat planning, systems, QA, and gamer-role agents | GPT-5 / Codex Desktop | @jonathanwxh-cell | Repaired stagger/Execute authority without changing balance: poise-break openings now survive same-hit phase thresholds, Execute owns central/semantic/touch truth until consumed, ordinary heavy resumes afterward without spending Resolve, and exact-duration, exact-once, real-touch, keyboard, and semantic-DOM regressions lock the contract |
| v2.27.6 | 2026-07-29 | Codex | GPT-5 / Codex Desktop | @jonathanwxh-cell | Added presentation-only king reactions for Sunder and Execute, removed duplicate impact-name text, corrected desktop next-action prompts to the real K key, and expanded real-touch, keyboard, render-purity, decay/reset, and screenshot acceptance without changing combat authority or balance |
| v2.27.7 | 2026-08-02 | Dev (Hermes) | deepseek-v4-flash | @jonathanwxh-cell | Fixed the QA port collision found in a whole-repo review: `qa/run.cjs` now picks a free ephemeral 127.0.0.1 port instead of hardcoding 8492 (the 849x block is allocated to other services on the production box), with `GRACEFELL_QA_PORT` still overriding and `GRACEFELL_URL` skipping the local server. Docs updated (README layout, info.md). Full QA passes with no env override — the exact case that used to EADDRINUSE |
...
| v2.29 local candidate | 2026-09-06 | Codex | GPT-5 / Codex Desktop | @jonathanwxh-cell | Responded to static-character feedback with real pelvis/knee/ankle rig pivots, travel-driven two-bone leg motion, breathing and body-led continuous action poses, fixed model-forward joint signs, preserved simulation and classic presentation, and added deterministic motion tests plus a labeled moving A/B audition; local only pending owner motion review |
| v2.29 publication | 2026-09-06 | Codex | GPT-5 / Codex Desktop | @jonathanwxh-cell | Prepared owner-authorized publication of the motion candidate through reviewed PR and CI, exact-SHA backup-first staged deployment to the existing shared Hetzner host, and public gameplay/GPU/input verification; deployment outcome receipts accompany the release PR |
