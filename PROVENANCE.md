...
| v2.27.2 | 2026-07-28 | Codex | GPT-5 / Codex Desktop | @jonathanwxh-cell | Fixed the focused combo review findings: pending Sunder can no longer steal stagger Execute, one deliberate input survives wound recovery with roll priority, contact-authoritative HUD/button states name the next action and explain route loss, pause retains technique recipes, deterministic gameplay plus phone-capture acceptance preserves the 600 ms window and all balance/save/audio/asset contracts, and repeated acceptance evidence hardened suspended-context charge cleanup plus permanent-deck reuse at the existing crossfade settle point |
