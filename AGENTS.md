...
- localStorage key `gracefell` — schema **v3**: `{v, bestTime, bests{}, wins, lastScore, bestScores{}, attempts, muted, grace, shakeEnabled, flashReduced, hapticsEnabled}`. `lastScore` and each `bestScores[grace]` entry preserve `{grade, time, trial, attempt, damageDealt, woundsTaken}`. Bump `Game.SAVE_VERSION` and add a migration branch in the constructor if you change it; there is a QA test that a v1 save still loads.
...
## Known scope edges
- Touch is now genuinely tested: `qa/verify.cjs` runs a third pass in a `hasTouch + isMobile` context that taps to start, verifies +5 swipe floors/visual timing and IRONBOUND disclosure, drives a real drag through the touch listeners, taps ATK, checks every button for overlap / off-screen / joystick-zone / fingertip-size violations, then dies naturally and proves both touchscreen and pointer-only resurrection. The desktop lane enumerates all nine grace settings and the immutable active snapshot. Dedicated `touch-forsaken-title.png` and `touch-forsaken-ironbound.png` artifacts guard the hardest-setting UI.
